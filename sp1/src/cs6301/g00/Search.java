...
public class Search {
	...
	/**
	 * Procedure to run binary search on a sorted array of integers Return true if
	 * there is p <= q <= r, with A[q] = x, and false otherwise Runs in time O(log
	 * n), where n = r-p+1
	 *
	 * @param A
	...
	public static boolean recursiveBinarySearch(int[] A, int p, int r, int x) {
		// Compare middle element of A[p..r] to x to decide which half of the array to
		// search
		if (p <= r) {
			int q = (p + r) / 2;
	...
	/**
	 * Implementation of Binary Search algorithm using generics. Use on any array of
	 * objects with a comparator that is supplied.
	 */
	...
	/**
	 * Implementation of Binary Search algorithm using generics. This can only be
	 * used on a type that implements Comparable itself. Item class can use it, but
	 * ItemExt will not.
	 */
	 ...
	 * that implements Comparable in its inheritance hierarchy
	 */
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < trials / 1000; i++) {
			int x = searchKey[i];
			if (linearSearch(arr, 0, size - 1, x)) {
				succ++;
			}
		}
		...
		for (int i = 0; i < trials; i++) {
			int x = searchKey[i];
			if (recursiveBinarySearch(arr, 0, size - 1, x)) {
				succ++;
			}
		}
		System.out.println("Successful searches: " + succ);
		...
		for (int i = 0; i < trials; i++) {
			int x = searchKey[i];
			if (iterativeBinarySearch(arr, 0, size - 1, x)) {
				succ++;
			}
		}
		System.out.println("Successful searches: " + succ);
		...
		for (int i = 0; i < trials; i++) {
			...
			if (binarySearch(iarr, 0, size - 1, x)) {
				succ++;
			}
		}
		System.out.println("Successful searches: " + succ);
 ...
 * Sample output:
 * 
 * Searching for 2000 elements: linear search... Successful searches: 169 Time:
 * 1494 msec. Memory: 57 MB / 124 MB.
 * 
 * Searching for 2000000 elements: recursive binary search... Successful
 * searches: 190750 Time: 414 msec. Memory: 57 MB / 124 MB.
 * 
 * Searching for 2000000 elements: iterative binary search... Successful
 * searches: 190750 Time: 397 msec. Memory: 57 MB / 124 MB.
 * 
 * Searching for 2000000 elements iteratively (generic)... Successful searches:
 * 190750 Time: 992 msec. Memory: 80 MB / 157 MB.
 */
